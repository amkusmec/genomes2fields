...
pA1 <- as_tibble(cmat1, rownames = "Phenotype1") %>%
  ...
  ggplot(., aes(x = Phenotype1, y = Phenotype2, fill = R)) + theme_classic() + 
    ...
    theme(axis.text.x = element_text(angle = 45, hjust = -0.0625, colour = fills), 
          axis.text.y = element_text(colour = rev(fills))) + 
    labs(x = "", y = "", fill = "", subtitle = "relative frequency = 36.2%", tag = "A")
...
pB1 <- tibble(Phenotype = phenotypes, 
  ...
  ggplot(., aes(x = Phenotype, y = V)) + theme_classic() + 
    ...
    labs(x = "", y = "Loadings", fill = "", tag = "B", 
         subtitle = paste0("Eigenvector 1, (PVE = ", 
                           round(svd1$d[1]^2/sum(svd1$d^2), 2)*100, "%)")) + 
    scale_fill_manual(values = fills) + 
    # scale_y_continuous(limits = c(-1, 1)) + 
    theme(axis.text.x = element_text(angle = 45, hjust = 1, colour = fills2)) + 
    guides(fill = FALSE)
...
gp1 <- arrangeGrob(ggplotGrob(pA1), ggplotGrob(pB1), ggplotGrob(pC1), 
                   layout_matrix = lay1)
ggsave("figures/select/mash_ED1_all.png", plot = gp1, width = 12, height = 4, 
       units = "in", dpi = 300)
...
pA2 <- as_tibble(cmat2, rownames = "Phenotype1") %>%
  ...
  ggplot(., aes(x = Phenotype1, y = Phenotype2, fill = R)) + theme_classic() + 
    ...
    theme(axis.text.x = element_text(angle = 45, hjust = -0.0625, colour = fills), 
          axis.text.y = element_text(colour = rev(fills))) + 
    labs(x = "", y = "", fill = "", subtitle = "relative frequency = 11.7%", tag = "A")
...
gp2 <- arrangeGrob(ggplotGrob(pA2), ggplotGrob(pB2), layout_matrix = lay2)
ggsave("figures/select/mash_ED2_all.png", plot = gp2, width = 10, height = 4, 
       units = "in", dpi = 300)
